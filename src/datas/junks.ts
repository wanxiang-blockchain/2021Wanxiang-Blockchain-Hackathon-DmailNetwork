    ...
    return Array.from(Array(200).keys()).map(id => {
        ...
        const status =
            isAfter(date, aMonthAgo) && random.boolean()
                ? 'junk'
                : weightedArrayElement(['junk'], [10, 1]);

        var titles: string[];
        titles = ["[spam] WelcomeToCarInsurance",
            "[spam] 50lbs in 61 Days",
            "[spam] 2 Days Left! Enjoy up to 30x Boost Coins!!",
            "[spam] Welcome to Apple Reward Panel",
            "[spam]Calling All Homeowners..."]
